...
{
    return (server->params->num_teams * 5) + count_all_words_size(array) + 2;
}
...
void send_name_of_all_the_teams(t_server *server, UNUSED char **array, int id)
{
    ...
    if (!message)
        return;
    for (size_t i = 0; server->params->team_names[i]; i++) {
        strncat(message, "tna ",strlen(message) + 4);
        strncat(
            message, server->params->team_names[i],
            strlen(message) + strlen(server->params->team_names[i])
        );
        strncat(message, "\n",strlen(message) + 1);
    }
